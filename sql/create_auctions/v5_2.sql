-- 5_2 added the update on auctions_sellers.time_per_round
CREATE OR REPLACE FUNCTION create_auction_v5_2 (auction jsonb) RETURNS UUID AS $$
...
BEGIN
  ...
  INSERT INTO auctions (
    ...
    VALUES (
      ...
      auction->>'name',
      COALESCE((lot->>'max_rank_displayed')::float8, 100),
      ...
      COALESCE((lot->>'rank_per_line_item')::boolean, false)
      )
      ON CONFLICT (id) DO UPDATE
      SET
        name = excluded.name,
        max_rank_displayed = excluded.max_rank_displayed,
        ...
        rank_per_line_item = excluded.rank_per_line_item
    ...
    --TODO: Faire le delete si (auction_id, seller_email) est present dans auctions_sellers mais pas
          ...
          -- Set time_per_round from lot.suppliersTimePerRound
          UPDATE auctions_sellers
          SET time_per_round = (
            SELECT (value->>'time_per_round')::float8
            FROM jsonb_array_elements(lot->'suppliersTimePerRound') AS value
            WHERE value->>'email' = supplier_price->'supplier'->>'email'
          )
          WHERE auction_id = new_lot_id
            AND seller_email = supplier_price->'supplier'->>'email';
