<template>
    ...
    <v-col cols="12" md="4" class="pb-0">
      <div class="pb-2">
        ...
        <v-text-field
          ...
          :rules="[numberHandler]"
          :suffix="props.basics.currency"
          :placeholder="t('lotRules.endingPrice.placeholder')"
        />
      </div>
      ...
      <div class="pb-2">
        ...
        <v-text-field
          :model-value="calculatedStartingPrice"
          variant="outlined"
          type="number"
          :rules="[numberHandler]"
          disabled
  ...
  <v-row>
    <v-col cols="12" class="py-0">
      <div class="text-body-2">Pre-bid</div>
...
const titles = {
  ...
  b: t('lotRules.roundIncrement'),
  c: t('lotRules.duration'),
  d: t('lotRules.roundDuration'),
  e: t('lotRules.endingPrice.label'),
  f: t('lotRules.startingPrice.label')
}

const schemaToRule = useZodSchema()
const numberRule = computed(() => {
  const numberSchema = z.number().min(0.01, { message: t('validation.positiveNumber') })
...
const calculatedStartingPrice = computed(() => {
  const firstRound = rounds.value[0]
  return firstRound ? firstRound.price : 0
})
...
const rounds = computed(() => {
  ...
  return Array.from({ length: nbRounds.value }, (r, i) => {
    return {
      price: model.value.max_bid_decr - (nbRounds.value - i - 1) * model.value.min_bid_decr,
      status: 'inactive'
    }
  })
})
