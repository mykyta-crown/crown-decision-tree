<template>
  ...
  <td>
    ...
    >
      <span>{{ status.label === 'upcoming' ? nbPassedRounds : nbPassedRounds + 1 }}</span>
    </v-chip>
  </td>
  ...
  <td>
    {{ formatNumber(rounds[nbPassedRounds]?.price, 'currency', auction.currency) }}
  </td>
  <td>
    <div class="d-flex align-center justify-center ga-4">
      <div class="text-no-wrap custom-width">
        {{
...
const { nbPassedRounds } = useDutchRounds(auction, bids)
...
const badgeColor = computed(() => {
  if (lowestBid.value.price > 0) {
    if (!colorsMap[lowestBid.value.supplier?.email]) {
      console.log('lowestBid: ', lowestBid.value)
...
const rounds = computed(() => {
  ...
  return Array.from({ length: maxNbRounds.value }, (r, i) => {
    const price = maxPrice - (maxNbRounds.value - i - 1) * auction.value.min_bid_decr
    return {
      price
    }
  })
