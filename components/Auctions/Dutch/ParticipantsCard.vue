            ...
            <td class="company-cell text-truncate pt-2">
              <div class="d-flex align-center">
                ...
                <div
                  ...
                  :style="{
                    'background-color': colorsMap[presence.email].secondary
                  }"
                >
                  <v-tooltip :text="presence.company">
                    <template #activator="{ props }">
                      <div v-bind="props" class="text-truncate px-2">
                        {{ presence.company }}
                      </div>
                    </template>
                  </v-tooltip>
...
// const props = defineProps(['auction', 'bids', 'bestBidsByCompanies'])
...
const bids = computed(() => {
  return auction.value.bids.filter((e) => e.type === 'bid')
})

const prebidsByEmail = computed(() => {
  return _(prebids.value)
    .filter((bid) => bid.profiles?.email)
...
// Check if this is a Dutch Preferred auction (at least one supplier has reduced time)
const isDutchPreferredAuction = computed(() => {
...
// Only relevant in Dutch Preferred auctions
...
const isPreferredSupplier = (email) => {
  if (!isDutchPreferredAuction.value) return false // No crown in regular Dutch auctions
...
const formatPresences = computed(() => {
  const formatted = auction.value.auctions_sellers.map((seller) => {
    const bidder = lowerBidsByComp.value?.find((e) => e.profiles.email === seller.seller_email)
